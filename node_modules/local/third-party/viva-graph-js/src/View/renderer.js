...
function renderer(graph, settings) {
  ...
  var layout = settings.layout,
    ...
    zoomSpeed = settings.zoomSpeed || 0.5,
  ...
  function createLabelUi(label) {
      var labelPosition = { x: 0, y: 0 };
      graphics.addLabel(label, labelPosition);
  }
  function removeLabelUi(label) {
      graphics.releaseLabel(label);
  }
  ...
  function initDom() {
    ...
    graph.forEachLabel(createLabelUi);
  ...
  function processLabelChange(change) {
      var label = change.label;
      if (change.changeType === 'add') {
          createLabelUi(label);
      } else if (change.changeType === 'remove') {
          removeLabelUi(label);
  ...
  function scale(out, scrollPoint) {
    ...
    if (!zoomSpeed) zoomSpeed = 0.5;
    var scaleFactor = Math.pow(1 + zoomSpeed, out ? -0.2 : 0.2);
  ...
  function stopListenToEvents() {
    ...
    graph.forEachLabel(function (label) {
        removeLabelUi(label);
    });
