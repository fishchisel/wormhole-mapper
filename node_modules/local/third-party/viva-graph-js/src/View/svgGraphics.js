...
/*jshint unused: false */
        ...
        labelBuilder = function (label) {
            var content = label.id;
            return svg("text").text(content)
                .attr("fill", "#999")
                .attr('font-size', '250%');
        },

        labelPositionCallback = function (labelUI, pos) {
            labelUI.attr("x", pos.x).attr("y", pos.y);
        },
    ...
    var allNodeUis = []; // used for inserting links in correct location
    ...
    var graphics = {
        ...
        label: function (builderCallback) {
            if (typeof builderCallback !== "function") {
                return; // todo: throw? This is not compatible with old versions
            }
            labelBuilder = builderCallback;
            return this;
        },
        ...
        placeLabel: function (newPlaceLabelCallback) {
            labelPositionCallback = newPlaceLabelCallback;
            return this;
        },
        ...
        */
        ...
        addLink: function (link, pos) {
            ...
            if (allNodeUis.length > 0) {
                svgContainer.insertBefore(linkUI, allNodeUis[0]);
            } else {
        ...
        **/
        releaseNode : function (node) {
            ...
            if (nodeUi) {
                ...
                allNodeUis.splice(allNodeUis.indexOf(nodeUi));
                delete allNodes[node.id];
            }
        },

        addLabel: function (label, pos) {
            var labelUi = labelBuilder(label);
            if (!labelUi) {
                return;
            }
            labelUi.position = pos;
            labelUi.label = label;
            allLabels[label.id] = labelUi;

            if (svgContainer.childElementCount > 0) {
                svgContainer.insertBefore(labelUi, svgContainer.firstChild);
            } else {
                svgContainer.appendChild(labelUi);
            }
        ...
        releaseLabel: function (label) {
            var labelUi = allLabels[label.id];
            if (labelUi) {
                svgContainer.removeChild(labelUi);
                delete allLabels[label.id];
            }
        },
        ...
        renderLabels : function () {
            for (var key in allLabels) {
                if (allLabels.hasOwnProperty(key)) {
                    var labelUi = allLabels[key];
                    cachedPos.x = labelUi.position.x;
                    cachedPos.y = labelUi.position.y;
                    labelPositionCallback(labelUi,cachedPos,labelUi.label);
                }
            }
        },
